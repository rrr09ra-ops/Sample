...
def get_missed(uid):
    ...
    missed = []
    for i in range((today_d - start).days + 1):
...
async def weekly_streak(app):
    ...
    for uid, username, name in users:
        ...
        text += f"{tag} — 🔥 {streak} | Missed: {','.join(missed[:5])}\n"
...
def main():
    ...
    # ✅ FIXED SCHEDULER (IMPORTANT)
    scheduler.add_job(lambda: run_async(app, reminder), 'cron', hour=14)
    scheduler.add_job(lambda: run_async(app, reminder), 'cron', hour=16)

    scheduler.add_job(lambda: run_async(app, lambda a: a.bot.send_message(GROUP_ID, build_report())), 'cron', hour=20)
